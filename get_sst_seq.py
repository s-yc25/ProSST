...
# 线程/进程映射函数
# ========================
def iter_parallel_map(func, data, workers: int = 1):
    """单线程版本替代多进程"""
    results = []
    for item in data:
...
def iter_threading_map(func, data, workers: int = 1):
    """单线程版本替代线程池"""
    results = []
    for item in data:
        results.append(func(item))
...
def threading_map(func, data, workers: int = 1):
    """单线程版本替代线程池 map"""
    results = []
    for item in data:
        results.append(func(item))
...
# 构建子图函数
# ========================
def subgraph_conventer(subgraph_dir, pdb_dir, max_batch_nodes, num_processes=1):
    ...
    def collate_fn(batch):
        ...
        prue_batch_graphs = [convert_graph(d) for d in batch_graphs]
